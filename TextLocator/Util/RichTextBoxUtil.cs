...
namespace TextLocator.Util
{
    public class RichTextBoxUtil
    {
        ...
        /// <param name="brush">高亮颜色画刷</param>
        /// <param name="underLine">下划线</param>
        public static void FillingDataFlowDocument(FlowDocumentReader reader, string text, Brush brush, bool underLine = false)
        {
            // Paragraph 类似于 html 的 P 标签
            Paragraph paragraph = new Paragraph();
            ...
            paragraph.Inlines.Add(run);
            reader.Document = new FlowDocument(paragraph);
        }
        ...
        public static void HighlightedFlowDocument(FlowDocumentReader reader, Color color, List<string> keywords, bool background = false)
        {
            ...
            foreach (string keyword in keywords)
            {
                ...
                TextPointer position = reader.Document.ContentStart;
                while (position != null)
                {
                    // 向前搜索,需要内容为Text                
                    if (position.GetPointerContext(LogicalDirection.Forward) == TextPointerContext.Text)
                    {
                        ...
                        if (AppConst.REGEX_SUPPORT_WILDCARDS.IsMatch(keyword))
                        {
                            ...
                            if (matches.Success)
                            {
                                ...
                                position = SelectaFlowDocument(reader, color, start, end, background);
                            }
                        }
                        else
                        {
        ...
        /// <returns></returns>
        private static TextPointer SelectaFlowDocument(FlowDocumentReader reader, Color color, TextPointer tpStart, TextPointer tpEnd, bool background)
        {
            TextRange range = reader.Selection;
